...
\begin{itemize}
    ...
    \item while(TRUE):
    \begin{itemize}
        \item an arbitrary message $(r, m) \in M$ is delivered (to recipient $r$, $m$ is the message content); (subject to the eventual delivery constraint)
...
\begin{itemize}
    ...
    \item[--] an ambiguous configuration if it is neither a 0- nor a 1-configuration.
\end{itemize}}
...
configurations.\\
\mlenma{An Initial Ambiguous Configuration}{For every deterministic protocol $\pi$
...
\mlenma{Extending an Ambiguous Sequence}{Fix a deterministic protocol $\pi$ that
